...
public class FeaturedTopic extends domain.Entity {
    ...
    @Transactional
    public static long getAllFeaturedTopicsTotal(String name, int rowsPerPage) {
        long totalPages = 0, size;
        if(name.trim().equals("")) {
            size = (long) JPA.em().createQuery("Select count(*) from FeaturedTopic a where deleted = false").getSingleResult();
        } else {
            Query query = JPA.em().createQuery("Select count(*) from FeaturedTopic a where deleted = false and a.name LIKE ?2");
    ...
    @Transactional
    public static List<FeaturedTopic> getAllFeaturedTopics(String name, int currentPage, int rowsPerPage, long totalPages) {
        int start=0;
        
        String sql="";
        if(name.trim().equals("")) {
            sql = "Select f from FeaturedTopic f where deleted = false order by publishedDate desc";
        } else {
            sql = "Select f from FeaturedTopic f where deleted = false and f.name LIKE ?1 order by publishedDate desc";
        }
        ...
        return (List<FeaturedTopic>)q.getResultList();
    }
}
