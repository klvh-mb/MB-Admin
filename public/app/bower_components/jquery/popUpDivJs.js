...
{
    $("#fade").fadeIn("fast");
    $("section#"+popDiv).fadeIn("fast")
}
function disablePopup(popDiv)
{
    $("#fade").fadeOut("fast");
    $("section#"+popDiv).fadeOut("fast")
}
function centerPopup(popDiv)
{
    var windowWidth=$(document).width();
    var windowHeight=$(document).height();
    var popupHeight=$("section#"+popDiv).height();
    var popupWidth=$("section#"+popDiv).width();
    arrPageScroll=___getPageScroll();
    arrPageSizes=___getPageSize();
    $("section#"+popDiv).css({"position":"absolute","top":(arrPageSizes[3]/ 2 - $("#" + compDiv).height() /2+getScrollTop())+"px","left":windowWidth/2-popupWidth/2,"z-Index":1002});
